	...
	if (!isset($_POST["mode"]) ){
		...
		if ($mode == "create"){ //if user chooses to create events
			...
			if ((!isset($_POST["date"])) || (!isset($_POST["time"])) || (!isset($_POST["name"])) ){
				...
				$stmt = $mysqli->prepare("INSERT INTO events(username, date, starttime, endtime, name, location) values (?, ?, ?, ?, ?)");
				...
				$stmt->bind_param("sssss", $username, $date, $starttime, $endtime, $name, $location);
				$stmt->execute();
				$stmt->close();
		...
		//if user wants to view events 
			...
			$stmt = $mysqli->prepare("SELECT id, date, starttime, endtime, name, location from events where date>=? and date<=? and username = ?");
			if(!$stmt){
			...
			$stmt->execute();
			$stmt->bind_result($id, $date, $starttime, $endtime, $name, $location);
			$events_array = array();

			while($stmt->fetch()){
				$id = htmlspecialchars($id);
				$date = htmlspecialchars($date);
				$starttime = htmlspecialchars($time);
				$endtime = htmlspecialchars($time);
